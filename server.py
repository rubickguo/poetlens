...
try:
    engine = create_engine(cfg.SQLALCHEMY_DATABASE_URI, **cfg.SQLALCHEMY_ENGINE_OPTIONS)
except Exception as e:
    print(f"Failed to create DB engine: {e}")
    # Fallback to local sqlite if path still exists? No, user wants migration.
    # raise SystemExit(1)
...
def _load_all_md_list() -> list[str]:
    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(text("SELECT exif_json FROM image_analysis")).fetchall()
        ...
        print(f"Error loading md list: {e}")
        return []
...
def _make_image_url(path_str: str) -> str:
    try:
        ...
        base_dir = Path(getattr(cfg, 'PHOTOS_BASE_DIR', '') or IMAGE_DIR)
        
        # Parse db path
        p = Path(path_str).expanduser().resolve()
        
        # If db_path has drive letter, strip it to simulate relative path
        parts = p.parts
        ...
        full_p = (base_dir / rel_path).resolve()
        
        # NOW check relative to IMAGE_DIR (assuming IMAGE_DIR is also pointing to correct filtered subset or just base?)
        # Actually server.py assumes IMAGE_DIR is the root of served images.
        # If PHOTOS_BASE_DIR is the new root, maybe we should just use that?
        # But `_make_image_url` returns `/images/...`.
        # Flask static serve likely maps `/images` to `IMAGE_DIR`.
        # So we need to ensure `rel` is correct relative to `IMAGE_DIR`.
        
        # If IMAGE_DIR is dynamic, it should be set in config using PHOTOS_BASE_DIR.
        # But here we just assume path_str might be old 'E:' and we are on 'Z:'.
        
        # Simple fix: return the relative path string, assuming Flask static handler knows where to look?
        # server.py doesn't seem to have a dynamic route for `/images/<path>`. It usually relies on nginx or `send_from_directory`.
        # Wait, I see no route for `/images/` in server.py snippets?
        # Ah, maybe I missed it.
        # Let's just update the path resolution logic to be safe.
        
        # If we can't find it relative to IMAGE_DIR, return empty.
        # But if we re-root it, maybe it IS relative to IMAGE_DIR.
        ...
        try:
             rel = full_p.relative_to(IMAGE_DIR.resolve())
             return "/images/" + str(rel).replace("\\", "/")
        except ValueError:
             # Try relative to PHOTOS_BASE_DIR if IMAGE_DIR is not parent
             # But web server must serve it.
             return ""
...
def serve_nas_photo(filename):
    ...
    full_path = os.path.join(app.config.get('PHOTOS_BASE_DIR', str(IMAGE_DIR)), decoded_filename)
    
    # 打印日志方便调试
    print(f"请求图片: {decoded_filename} | 完整路径: {full_path} | Base: {app.config.get('PHOTOS_BASE_DIR', str(IMAGE_DIR))}")
    
    if not os.path.exists(full_path):
        # Fallback: check if we need to join with PHOTOS_BASE_DIR differently 
        # (user logic assumes full join, but filename might be relative)
        print(f"文件未找到: {full_path}")
        return f"文件未找到: {full_path}", 404
        
    return send_from_directory(app.config.get('PHOTOS_BASE_DIR', str(IMAGE_DIR)), decoded_filename)
...
def load_rows(page: int = 1, page_size: int = REVIEW_PAGE_SIZE, md: str = "", sort: str = "memory"):
    ...
    # MySQL specific text extraction if needed, but we do it in python or use generic params
    # SQLite: json_extract(exif_json, '$.datetime')
    # MySQL: json_unquote(json_extract(exif_json, '$.datetime')) or column->>'$.datetime'
    # We will try to keep it standard SQL or use Python filtering if complex? 
    # But paging needs SQL.
    # Note: image_analysis table has 'exif_datetime' column! We should use that instead of parsing JSON.
    
    dt_col = "exif_datetime" # Use the column provided by new schema
    
    # MD expr: substr(dt, 6, 2) || '-' || substr(dt, 9, 2)
    # MySQL substring is similar. || is OR in MySQL (unless PIPES_AS_CONCAT enabled). Use CONCAT.
    # CONCAT(SUBSTR(exif_datetime, 6, 2), '-', SUBSTR(exif_datetime, 9, 2))
    
    md_expr = f"CONCAT(SUBSTR({dt_col}, 6, 2), '-', SUBSTR({dt_col}, 9, 2))"
    ...
    if md and len(md) == 5 and md[2] == "-":
        where_clauses.append(f"{dt_col} IS NOT NULL AND {md_expr} = :md")
        params["md"] = md
    ...
    with engine.connect() as conn:
        # Total count
        count_sql = text(f"SELECT COUNT(1) FROM image_analysis WHERE {where_sql}")
        ...
        if sort == "beauty":
            ...
            order_sql = f"ORDER BY ({dt_col} IS NULL) ASC, {dt_col} DESC, path"
        elif sort == "time_old":
            order_sql = f"ORDER BY ({dt_col} IS NULL) ASC, {dt_col} ASC, path"
        else:
            order_sql = "ORDER BY COALESCE(memory_score, -1) DESC, COALESCE(beauty_score, -1) DESC, path"

        # Map new columns (one_sentence_copy) to old expected names (side_caption)
        # Handle 'used_at' as NULL
        base_sql = text(f"""
            SELECT path,
...
def load_sim_rows_for_dates(dates: list[str]):
    ...
    # Using exif_datetime column direct format YYYY:MM:DD HH:MM:SS
    # We want to match YYYY-MM-DD
    # date(exif_datetime) in MySQL extracts date part YYYY-MM-DD? yes.
    # Or substr string match.
    # original logic: replace(substr(json..., 1, 10), ':', '-')
    
    # Let's use string param expansion since list length varies
    placeholders = ",".join([f":d{i}" for i in range(len(safe_dates))])
    params = {f"d{i}": d for i, d in enumerate(safe_dates)}
    ...
    sql = text(f"""
        ...
        WHERE exif_datetime IS NOT NULL
          AND REPLACE(SUBSTR(exif_datetime, 1, 10), ':', '-') IN ({placeholders})
    """)
...
def index():
    ...
    return Response("poetlens server running. WebUI disabled.", mimetype="text/plain; charset=utf-8")
...
@app.get("/static/poetlens/<key>/photo_<int:idx>.bin")
...
@app.get("/static/poetlens/<key>/latest.bin")
...
@app.get("/static/poetlens/<key>/preview.png")
...
if __name__ == "__main__":
    ...
    mimetypes.add_type("application/octet-stream", ".bin")
    print(f"[poetlens] DB Config: {cfg.SQLALCHEMY_DATABASE_URI.split('@')[-1]}") # Show host only for privacy
    print(f"[poetlens] IMAGE_DIR: {IMAGE_DIR}")
    print(f"[poetlens] OUT: {BIN_OUTPUT_DIR}")
    print("[poetlens] key: [hidden]")
    print(f"[poetlens] listen: {FLASK_HOST}:{FLASK_PORT}")
    print(f"[poetlens] open: http://127.0.0.1:{FLASK_PORT}/  (本机)")
